...
const ResultDisplay: React.FC<ResultDisplayProps> = ({ result, onMarkHit, onManualRectify }) => {
    ...
    // Placeholder caso não haja resultado
        ...
        <div className="bg-slate-900/60 p-4 rounded-[2rem] border border-cyan-900/30 shadow-2xl relative backdrop-blur-xl flex flex-col gap-3">
            <div className="flex items-center justify-between px-2">
                <div className="flex items-center gap-2">
                    <div className={`w-2.5 h-2.5 rounded-full shadow-[0_0_10px_#06b6d4] ${result ? 'bg-cyan-500' : 'bg-slate-700'}`}></div>
            ...
            <div className="grid grid-cols-4 sm:grid-cols-7 gap-2">
                {displayData.map((row, idx) => {
                    ...
                    return (
                        <div key={idx} className={`group relative p-2.5 rounded-2xl border flex flex-col items-center justify-center transition-all ${isTop ? 'bg-amber-500/5 border-amber-600/30' : 'bg-slate-900/50 border-slate-800/80'} ${!result && 'opacity-40'}`}>
                            ...
                            <button 
                                ...
                                className={`p-1.5 rounded-xl relative z-[45] ${!result ? 'bg-slate-800 text-slate-700 cursor-not-allowed' : (isTop ? 'bg-amber-500 text-slate-950 shadow-md' : 'bg-slate-800 text-slate-400')}`}
                            >
                            ...
                            {openMenu === idx && (
                                <div onClick={(e) => e.stopPropagation()} className="fixed md:absolute top-1/2 left-1/2 md:top-[-20px] md:left-1/2 -translate-x-1/2 -translate-y-1/2 md:translate-y-0 w-[90vw] max-w-[240px] bg-slate-950 rounded-[2rem] z-[9999] p-5 flex flex-col gap-3 border-2 border-amber-500 shadow-[0_0_50px_rgba(245,158,11,0.7)] animate-in zoom-in duration-200">
                                    ...
                                    <input ref={inputRef} type="text" placeholder="VALOR REAL" value={localVal} onChange={(e) => setLocalVal(e.target.value.replace(/\D/g, ''))} className="w-full bg-slate-900 border border-slate-800 text-center font-orbitron text-[20px] py-3 rounded-2xl text-amber-500 outline-none focus:border-amber-500 placeholder:text-slate-800" maxLength={idx === 6 ? 3 : 4} inputMode="numeric" />
                                    <div className="flex flex-col gap-2">
                                        <button onClick={() => { onMarkHit(value, type, idx + 1, 'Acerto'); setOpenMenu(null); }} className="w-full bg-amber-500 text-slate-950 py-3.5 rounded-2xl text-[9px] font-black uppercase tracking-widest active:scale-95">SALVAR ACERTO</button>
                                        <button onClick={() => { onMarkHit(value, type, idx + 1, 'Quase Acerto'); setOpenMenu(null); }} className="w-full bg-slate-900 border border-amber-500/50 text-amber-500 py-3 rounded-2xl text-[8px] font-black uppercase tracking-widest active:scale-95">QUASE ACERTO</button>
                                        <button onClick={() => { if (localVal) { onManualRectify(value, localVal, type, rankLabel); setOpenMenu(null); } }} className="w-full bg-amber-900/40 text-amber-200 py-3 rounded-2xl text-[8px] font-black uppercase tracking-widest active:scale-95">SALVAR REALIDADE</button>
                                    </div>
                                </div>
                            )}
                        </div>
