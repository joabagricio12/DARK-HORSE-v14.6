...
const AdvancedPredictionDisplay: React.FC<AdvancedPredictionDisplayProps> = ({ predictions, onMarkHit, onManualRectify }) => {
    ...
    return (
        <div className="flex flex-col gap-4 h-full relative">
            <div className="bg-slate-900 rounded-[2.5rem] border-2 border-amber-500 relative z-[40] shadow-[0_0_40px_rgba(245,158,11,0.5)] overflow-hidden">
                <div className="bg-slate-950/80 p-5 rounded-[2.4rem] relative overflow-hidden">
                    <h2 className="text-[11px] font-orbitron font-black text-amber-500 uppercase tracking-[0.4em] mb-4 flex items-center gap-2 justify-center">
                        <span className={`w-2.5 h-2.5 rounded-full shadow-[0_0_15px_#f59e0b] ${predictions ? 'bg-amber-500 animate-pulse' : 'bg-slate-800'}`}></span>
                        DEZENAS ELITE
                    </h2>
                    <div className="grid grid-cols-2 gap-4">
                        {eliteData.map((item, idx) => (
                            <div key={idx} className={`flex flex-col items-center bg-gradient-to-b from-amber-500/10 to-transparent p-5 border border-amber-500/20 rounded-[2.2rem] relative ${!predictions && 'opacity-30'}`}>
                                ...
                                <button 
                                    ...
                                    className={`w-full py-3 rounded-xl text-[10px] font-black uppercase shadow-[0_4px_0_#b45309] active:translate-y-[2px] active:shadow-none transition-all ${!predictions ? 'bg-slate-800 text-slate-600 shadow-none cursor-not-allowed' : 'bg-amber-500 text-slate-950'}`}
                                >
                                    MARCAR
                                </button>
            ...
            <div className={`bg-slate-900/60 p-5 rounded-[2.5rem] border border-slate-800/60 backdrop-blur-xl relative transition-all ${activeMenu?.id.includes('triad-d') ? 'z-[999]' : 'z-[30]'} ${!predictions && 'opacity-50'}`}>
                    ...
                    {superTensData.map((item, idx) => (
                        <div key={idx} className="flex flex-col items-center bg-slate-950/60 p-3 rounded-[1.8rem] relative">
                            ...
                            <button 
                                ...
                                className={`p-3 rounded-xl border border-slate-700 active:scale-90 ${!predictions ? 'bg-slate-900 text-slate-800' : 'bg-slate-800 text-slate-400'}`}
                            >
            ...
            <div className={`bg-slate-900/60 p-5 rounded-[2.5rem] border border-slate-800/60 backdrop-blur-xl flex-1 relative transition-all ${activeMenu?.id.includes('triad-c') ? 'z-[999]' : 'z-[20]'} ${!predictions && 'opacity-50'}`}>
                    ...
                    {hundredsData.map((item, idx) => (
                        <div key={idx} className="flex justify-between items-center bg-slate-950/60 p-4 rounded-[1.8rem] relative border border-slate-800/40">
                            ...
                            <button 
                                ...
                                className={`p-4 rounded-2xl border border-slate-700 active:scale-90 ${!predictions ? 'bg-slate-900 text-slate-800' : 'bg-slate-800 text-slate-400'}`}
                            >
