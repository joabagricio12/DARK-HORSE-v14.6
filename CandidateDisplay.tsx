...
const CandidateDisplay: React.FC<CandidateDisplayProps> = ({ candidates, onMarkHit, onManualRectify }) => {
    ...
    const displayCandidates = candidates?.slice(0, 3) || [{sequence: [0,0,0,0]}, {sequence: [0,0,0,0]}, {sequence: [0,0,0,0]}];
        ...
        <div className="bg-slate-900/80 p-4 rounded-[2rem] border border-slate-800/60 flex flex-col h-full shadow-inner backdrop-blur-md relative">
            ...
            <div className="flex-1 space-y-2.5">
                {displayCandidates.map((candidate, index) => {
                    ...
                    return (
                        <div key={index} className={`flex items-center justify-between bg-slate-950/60 p-4 border border-slate-800/80 rounded-[1.5rem] relative ${!candidates && 'opacity-40'}`}>
                            ...
                            <button 
                                ...
                                className={`p-4 rounded-2xl z-[120] border border-slate-700 active:scale-90 ${!candidates ? 'bg-slate-900 text-slate-800' : 'bg-slate-800/50 text-slate-400'}`}
                            >
